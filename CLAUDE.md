...
- Price: first from the mempool.space WebSocket if it has pushed the rate recently (`conversions`; `_price_ws_ts` / `PRICE_WS_FRESH_S`) — that socket is already open for the fee tile, so it costs nothing and reaches networks that intercept the dedicated price APIs. Otherwise a chain of keyless sources tried in order until the displayed currency has a quote (`_PRICE_SOURCES`, then Binance): CoinGecko → mempool.space `/api/v1/prices` → `blockchain.info/ticker` → Coinbase exchange-rates → Binance. mempool.space does not quote TRY or RUB, so those still depend on the other sources. The first three each cover every currency in one request; Binance is per-symbol and answers HTTP 451 to whole countries, so it is last and abandons the host on that status. `CURRENCIES` carries `None` for pairs Binance has delisted (GBP, RUB) — requesting them is a guaranteed `-1121 Invalid symbol`. `_http_get` returns `(body, status)` so callers can tell a geo-block from a bad symbol from no network. Polled on the user's refresh interval (1/5/15/30/60 min; `REFRESH_OPTIONS`) — but a cycle that fetched no live price reschedules on a short escalating backoff instead (`FETCH_RETRY_MIN_S`..`FETCH_RETRY_MAX_S`, see `_next_fetch_delay`). Windows refuses sockets (WinError 10013) for the first seconds of a process's life while the firewall clears the new binary, so the startup fetch routinely fails on a healthy machine; without the retry a 60-min interval left the price blank for an hour.
